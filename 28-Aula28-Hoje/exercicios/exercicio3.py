...
# 1) Crie uma função que receba como parametro a lista e retorne 1 lista em que a 1 possua todas as listas
...
def funcao (lista):
    ...
    for i in lista:
        if i[3] == 'f':
            ...
            lista_masc.append(i)
    lista_ord.append(lista_fem)
    print(f'{lista_fem}\n A quantidade de Mulheres é de: {len(lista_fem)}\n')
    lista_ord.append(lista_masc)
    print(f'{lista_masc}\nA quantidade de Homens é de: {len(lista_masc)}\n')

    return lista_ord

lista_completa = funcao(lista)   
